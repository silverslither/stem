...
# returns (s, r^2, (x, y, z)) or None if input is not a valid triangle
def variable_change(sides: list[Fraction]):
    ...
    return s, rr, (variables[0], variables[1], variables[2])
...
# return a rational solution for y if (x + y)(1 + 1/(xy/r^2 - 1)) = s if it exists, given x
# otherwise return -1 if no solution exists, or 0 if irrational solution(s) exist
# if (x + y)(1 + 1/(xy/r^2 - 1)) = s, y = (+/-sqrt(x(x(s - x)^2 - 4r^2s)) + sx - x^2)/(2x)
def get_rational_y(s: Fraction, rr: Fraction, x: Fraction):
    d = x * (x * (s - x) ** 2 - 4 * rr * s)
    if d < 0:
        return -1
    if d == 0:
...
def searcher(s: Fraction, rr: Fraction, depth: int):
...
# implicit differentiation of (x + y)(1 + 1/(xy/r^2 - 1)) = s gives dy/dx = -(y(y(x^2 - r^2) - 2r^2x))/(x(x(y^2 - r^2) - 2r^2y))
# returns (a, b) where the equation of tangent line is y = ax + b or None if slope is not valid
def find_tangent_expr(point: tuple[Fraction, Fraction], rr: Fraction):
    x, y = point
    m = y * (y * (x * x - rr) - 2 * rr * x)
    n = x * (x * (y * y - rr) - 2 * rr * y)
...
def find_secant_expr(point1: tuple[Fraction, Fraction], point2: tuple[Fraction, Fraction], rr: Fraction):
    if point1 == point2:  # handle tangent case automatically
        return find_tangent_expr(point1, rr)
    x1, y1 = point1
    x2, y2 = point2
    m = y2 - y1
    n = x2 - x1
...
# return a new (reflected) solution to (x + y)(1 + 1/(xy/r^2 - 1)) = s, given two existing ones
# if (x + y)(1 + 1/(xy/r^2 - 1)) = s and y = ax + b, (a^2 + a)x^3 + (2ab - as + b)x^2 + (b^2 - bs)x + r^2s = 0
# hence if solutions p, q (not necessarily unique) are known, the new solution is x = -r^2s/(pqa(a + 1)), y = -r^2s/(pq(a + 1)) + b
def get_new_solution(s: Fraction, rr: Fraction, line: tuple[Fraction, Fraction], solutions: tuple[Fraction, Fraction]):
    temp = -s * rr / (solutions[0] * solutions[1] * (line[0] + 1))
    return (temp + line[1], temp / line[0])


def solver_recurse(s: Fraction, rr: Fraction, basis: list[tuple[Fraction, Fraction]], depth: int, start: tuple[Fraction, Fraction] | None) -> Generator[tuple[Fraction, Fraction], None, None]:
    ...
    new_basis = basis[1:]
    yield from solver_recurse(s, rr, new_basis, depth, start)

    for i in range(1, depth + 1):
        if start == None:
            start = basis[0]
        else:
            line = find_secant_expr(start, basis[0], rr)
            if line == None:
                return
            start = get_new_solution(s, rr, line, (start[0], basis[0][0]))
        yield from solver_recurse(s, rr, new_basis, depth - i, start)


# solver step using chord and tangent methods up to a maximum Manhattan distance of depth in Z^n, with n basis points
def solver(s: Fraction, rr: Fraction, basis: Iterable[tuple[Fraction, Fraction]], depth: int):
    depth += 1
    triples: set[tuple[Fraction, Fraction, Fraction]] = set()
    for x, y in solver_recurse(s, rr, list(basis), depth, None):
        z = (x + y) / (x * y / rr - 1)
...
def reducer(s: Fraction, rr: Fraction, inputs: Iterable[tuple[Fraction, Fraction, Fraction]], depth: int):
    sorted_inputs = sorted(inputs, key=height)

    superset = sorted_inputs
    for i in range(1, len(sorted_inputs)):
        result = solver(s, rr, [(x[0], x[2]) for x in sorted_inputs[:i]], depth)
    ...
    while flag:
        for i in range(len(superset) - 1, -1, -1):
            ...
            del generators[i]
            result = solver(s, rr, [(x[0], x[2]) for x in generators], depth)
            if all([x in result for x in inputs]):
                superset = generators
                flag = True
                break
...
def main() -> int:
    ...
    while i < len(sys.argv) and "," in sys.argv[i]:
        ...
        if variables == None:
            ...
            return 1
        [_s, _rr, variables] = variables
        inputs.add(variables)
        if (s != None and s != _s) or (rr != None and rr != _rr):
    ...
    if s == None or rr == None:
        print("usage: solver.py <a,b,c [d,e,f] ...> [search depth = 12] [reducer depth = 8] [solver depth = 8] [-v]")
        return 1

    inputs = inputs.union(searcher(s, rr, searcher_depth))
    if verbose_printing:
    ...
    generators = reducer(s, rr, inputs, reducer_depth)
    if verbose_printing:
    ...
    basis = [(x[0], x[1]) for x in generators]
    solutions = solver(s, rr, basis, solver_depth)
